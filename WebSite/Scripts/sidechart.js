var side_height = 100
var x_time = d3.scale.linear().range([0, side_width])
var line_scale = {
...
function linechart(axis) {
    ...
    cliparea.selectAll('clip_step')
        .data(tmp).enter()
        .append('rect')
        .attr('class', function(d, i) {
            return 'clip_step clip_step' + i
        })
        .attr('x', function(d, i) {
            return side_width / (configdata.year.max - configdata.year.min) * i
        })
        .attr('y', y)
    ...
    if (axis == 'x') {
        updateLineDim(dx, axis)
    } else if (axis == 'y') {
        updateLineDim(dy, axis)
    }
}
